document.addEventListener('DOMContentLoaded', function () {
    ...
    function limpiarFiltrosSiHayBusqueda() {
        const texto = document.getElementById('input-busqueda').value.trim();

        if (texto !== '') {
            if (slimEstado) slimEstado.setSelected([]);
            if (slimDirector) slimDirector.setSelected([]);
            if (slimAnio) slimAnio.setSelected([]);
    ...
    // En esta parte se cargar resultados
    function cargarResultados() {
        ...
        // se obtiene la línea de investigación
        ...
        fetch('/repositorio_miidt/tesis-miidt/busqueda_ajax_resultados.php?' + params.toString())
            .then(response => {
                if (!response.ok) {
    ...
    // Hace la busqueda más eficiente al esperar a que el usuario termine de escribir antes de hacer la consulta AJAX
    let timeoutBusqueda;
